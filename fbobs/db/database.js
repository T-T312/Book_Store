...
const DB_PATH = path.join(__dirname, 'fbobs.db');
...
async function initDB() {
  ...
  const existing = db.exec("SELECT id FROM users WHERE email='admin@fbobs.com'");
  ...
  // Seed sample books
  const books = db.exec("SELECT id FROM books LIMIT 1");
  if (!books.length || !books[0].values.length) {
    ...
    sampleBooks.forEach(b => {
      db.run("INSERT INTO books (title, author, isbn, genre, price, stock, description) VALUES (?,?,?,?,?,?,?)", b);
    });
  }
...
// Helper: run insert/update/delete, save to disk
